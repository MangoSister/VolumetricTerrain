Shader "PCGTerrain/TriplanarMultiMatUv" {
	Properties {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandardSpecular o) 
		{
			...
			o.Albedo += triplanar_blend_weight.z * (
								splat_blend_weight.r * tex2D(_ColorTexR, IN.worldPos.xy) * tex2D(_ColorTexR, IN.worldPos.xy * -_UvOctave.r) * _BrightnessComp.r + 
								splat_blend_weight.g * tex2D(_ColorTexG, IN.worldPos.xy) * tex2D(_ColorTexG, IN.worldPos.xy * -_UvOctave.g) * _BrightnessComp.g +
								splat_blend_weight.b * tex2D(_ColorTexB, IN.worldPos.xy) * tex2D(_ColorTexB, IN.worldPos.xy * -_UvOctave.b) * _BrightnessComp.b +
								splat_blend_weight.a * tex2D(_ColorTexA, IN.worldPos.xy) * tex2D(_ColorTexA, IN.worldPos.xy * -_UvOctave.a) * _BrightnessComp.a 
								);

			//Color YZ plane
			o.Albedo += triplanar_blend_weight.x * (
								splat_blend_weight.r * tex2D(_ColorTexR, IN.worldPos.yz) * tex2D(_ColorTexR, IN.worldPos.yz * -_UvOctave.r) * _BrightnessComp.r +
								splat_blend_weight.g * tex2D(_ColorTexG, IN.worldPos.yz) * tex2D(_ColorTexB, IN.worldPos.yz * -_UvOctave.g) * _BrightnessComp.g +
								splat_blend_weight.b * tex2D(_ColorTexB, IN.worldPos.yz) * tex2D(_ColorTexG, IN.worldPos.yz * -_UvOctave.b) * _BrightnessComp.b +
								splat_blend_weight.a * tex2D(_ColorTexA, IN.worldPos.yz) * tex2D(_ColorTexA, IN.worldPos.yz * -_UvOctave.a) * _BrightnessComp.a 
								);
			//Color XZ plane
			o.Albedo += triplanar_blend_weight.y * (
								splat_blend_weight.r * tex2D(_ColorTexR, IN.worldPos.xz) * tex2D(_ColorTexR, IN.worldPos.xz * -_UvOctave.r) * _BrightnessComp.r +
								splat_blend_weight.g * tex2D(_ColorTexG, IN.worldPos.xz) * tex2D(_ColorTexG, IN.worldPos.xz * -_UvOctave.g) * _BrightnessComp.g +
								splat_blend_weight.b * tex2D(_ColorTexB, IN.worldPos.xz) * tex2D(_ColorTexB, IN.worldPos.xz * -_UvOctave.b) * _BrightnessComp.b +
								splat_blend_weight.a * tex2D(_ColorTexA, IN.worldPos.xz) * tex2D(_ColorTexA, IN.worldPos.xz * -_UvOctave.a) * _BrightnessComp.a
								);
